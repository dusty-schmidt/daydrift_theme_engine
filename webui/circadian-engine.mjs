...
export function buildDailyDrift(localDate, timezone = 'local') {
  ...
  return {
    ...
    // Let the immersive day-to-day variation show up in accents.
    accentHue: round((random() * 2 - 1) * 14, 3),
    accentSaturation: round((random() * 2 - 1) * 0.075, 4),
    accentLightness: round((random() * 2 - 1) * 0.035, 4),
